	...
	public static Collection<Object[]> validScenarios() throws ActivityDiagramRuleException {
		return Arrays.asList(new Object[][] {
	        ...
	        { Arrays.asList(
	        		...
	        		new TransitionTempHolder(NAME + "2", PROB, mergeNode, finalNode)), NAME, 1},
	        { Arrays.asList(
	        		...
	        		new TransitionTempHolder(NAME + "3", PROB, decisionNode, mergeNode)), NAME + "2",2},
	        { Arrays.asList(
	        		new TransitionTempHolder(NAME, PROB, activityNode, mergeNode),
	        		new TransitionTempHolder(NAME + "2", PROB, new ActivityNode("activity2"), mergeNode),
	        		new TransitionTempHolder(NAME + "3", PROB, decisionNode, mergeNode)), "invalidName",3},
		});
	}
	
	@ParameterizedTest
	@MethodSource("validScenarios")
	public void testRemoveTransitions(List<TransitionTempHolder> values, String nameToRemove, int expectedCount) throws ActivityDiagramRuleException {
		...
		diagramTransitions.removeTransition(nameToRemove);
		
		assertEquals(expectedCount, diagramTransitions.getTransitions().size());
	}
}
